...
async function executeOrderAsSolver(
  ...
  try {
    ...
    console.log(`Current auction price: ${ethers.utils.formatEther(currentPrice)} ETH`);
...
async function startSolverMonitor(
  ...
  console.log(`Solver monitor started. Checking every ${monitorInterval / 1000} seconds.`);
}
...
async function main(): Promise<void> {
  ...
  if (command === 'create-order') {
    ...
    if (args.length < 3) {
      console.log('Usage: npm run fusion-cross-chain create-order <private_key> <output_file>');
      process.exit(1);
    }
    ...
    // Check if we have required args
    if (args.length < 3) {
      console.log('Usage: npm run fusion-cross-chain start-solver <private_key> <orders_file>');
      process.exit(1);
    }
